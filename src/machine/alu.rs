...
impl Alu {
    ...
    pub fn execute_operator(&mut self, operator: AluOperator, a: u64, b: u64) -> u64 {
        let (result, negative, zero, carry, overflow) = match operator {
            ...
            AluOperator::Div => {
                if b == 0 {
                    (0u64, Some(false), Some(true), Some(true), Some(false))
                } else {
                    let (a_i, b_i) = (a as i64, b as i64);
                    if a_i == i64::MIN && b_i == -1 {
                        (a, Some(a >> 63 == 1), Some(a == 0), Some(false), Some(true))
                    } else {
            ...
            AluOperator::Rem => {
                if b == 0 {
                    ...
                    let (a_i, b_i) = (a as i64, b as i64);
                    if a_i == i64::MIN && b_i == -1 {
                        (a, Some(false), Some(true), Some(false), Some(true))
                    } else {
                        let result = (a_i % b_i) as u64;
                        (
                            result,
            ...
            AluOperator::Trl => (a, None, None, None, None),
            AluOperator::Trr => (b, None, None, None, None),
        };
        self.set_flags(negative, zero, carry, overflow);
        result
    }
